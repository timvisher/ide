...
# `aishot git diff review`, block dismissal phrasing per the GOLDEN RULE.
#
# `aishot git diff review --help` says:
...
# include an `aishot git diff review` Bash tool call. Otherwise the
# dismissal vocabulary is just normal English.
...
# Was `aishot git diff review` run anywhere in the recent transcript
# window? Matches the review subcommand only — `aishot git diff describe`
# is a factual changelog with no GOLDEN RULE, so it must not trip this.
RECENT_REVIEW=$(tail -n 200 "$TP" 2>/dev/null \
  | jq -rs '
    [.[] | select(.type=="assistant") | (.message.content // []) | .[]?
      | select(.type=="tool_use" and .name=="Bash"
               and ((.input.command // "") | test("aishot git diff review")))]
    | length' \
  2>/dev/null)
