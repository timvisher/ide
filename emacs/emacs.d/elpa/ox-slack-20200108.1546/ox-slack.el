...
(defun org-slack-link (link contents info)
  ...
  (let ((type (org-element-property :type link)))
    (if (member type (list "https" "http" "ftp" "mailto"))
        (let ((raw-link (org-element-property :raw-link link)))
          (if contents
              (format "[%s](%s)" contents raw-link)
            raw-link))
      (progn
        ;; One of the things that can cause this is a 'raw' internal link
        ;; to another heading, for instance. Like:
        ;;
        ;; ```
        ;; [[*Quick Cem update]]
        ;; ```
        ;;
        ;; That's such a pathological case that I'm OK with leaving the
        ;; assert here since that's almost certainly an error.
        (cl-assert contents t "link contents is falsey which may indicate that you have a raw internal link of some kind")
        (format "_%s_ 🔗" contents)))))

(timvisher--comment
 (setq timvisher-link nil)
 (setq timvisher-contents nil)
 (setq timvisher-info nil)

 (defun org-slack-link (link contents info)
   "Transcode LINK object into Markdown format.
  CONTENTS is the link's description.  INFO is a plist used as
  a communication channel."
   (setq timvisher-link link)
   (setq timvisher-contents contents)
   (setq timvisher-info info)
   (let* ((link timvisher-link)
          (contents timvisher-contents)
          (info timvisher-info)
          (type (org-element-property :type link)))

     (if (member type (list "https" "http" "ftp" "mailto"))
         (format "[%s](%s)" contents (org-element-property :raw-link timvisher-link)))
