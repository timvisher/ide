...
If called from Lisp, toggle the mode if ARG is `toggle'.  Enable the mode
if ARG is nil, omitted, or is a positive number.  Disable the mode if ARG
...
`Org-List-Checkbox-Radio mode' mode.  If the prefix argument is positive,
...
This is a minor mode.  If called interactively, toggle the `Org-Num mode'
